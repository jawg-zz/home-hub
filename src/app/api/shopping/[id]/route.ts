...
export async function PATCH(
  ...
  try {
    ...
    const item = await monitoring.trackPerformance(
      ...
      async () => prisma.shoppingItem.update({
        where: { id },
        data: validated,
      }),
      { requestId, userId: session.user.id, itemId: id }
    )
    ...
    if (error instanceof z.ZodError) {
      logger.warn('Invalid shopping item update data', { requestId, errors: error.issues })
      return NextResponse.json(
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2025') {
      return NextResponse.json(
        createErrorResponse('Item not found', 404),
        { status: 404 }
      )
    }
