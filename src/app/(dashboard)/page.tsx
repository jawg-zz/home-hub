    ...
    return (
      <div className="dashboard-container fade-in">
        ...
        <header className="welcome-header">
          <div className="greeting-section">
            <h1 className="welcome-title" style={{ fontWeight: 700 }}>
              Welcome back,{" "}
              <span className="highlight">{session?.user?.name || "Home"}</span>{" "}
              👋
            </h1>
        ...
        <section className="stats-grid">
          <div
            className="stat-card primary-stat slide-in"
          ...
          >
            ...
            <div className="stat-footer">
              <span
                className={`status-indicator ${onlineDevices > 0 ? "status-active" : "status-inactive"}`}
              ></span>
          ...
          <div
            className="stat-card secondary-stat slide-in"
          ...
          >
            ...
            <div className="stat-footer">
              <span
                className={`status-indicator ${shoppingItems > 0 ? "status-warning" : "status-success"}`}
              ></span>
              <span className="footer-text">
                {shoppingItems > 0
          ...
          <div
            className="stat-card warning-stat slide-in"
          ...
          >
            ...
            <div className="stat-value">
              <span
                className={`${pendingChores > 0 ? "warning-text" : "success-text"}`}
              >
                {pendingChores}
              </span>
            </div>
            <div className="stat-footer">
              <span
                className={`status-indicator ${pendingChores > 0 ? "status-warning" : "status-success"}`}
              ></span>
              <span className="footer-text">
                {pendingChores > 0
          ...
          <div
            className="stat-card neutral-stat slide-in"
          ...
          >
            ...
            <div className="stat-value">
              <span className="neutral-text">
                {energyToday?.usage.toFixed(1) || "0"}
              </span>
              <span className="stat-unit">kWh</span>
            </div>
            <div className="stat-footer">
              <span
                className={`status-indicator ${energyToday?.usage && energyToday.usage > 20 ? "status-warning" : "status-success"}`}
              ></span>
              <span className="footer-text">
                {energyToday?.usage && energyToday.usage > 20
                  ? "High usage"
                  : "Normal consumption"}
              </span>
            </div>
        ...
        <div className="main-grid">
          ...
          <section className="panel quick-actions-panel">
            <div className="panel-header">
              <h2 className="panel-title" style={{ fontWeight: 700 }}>
                Quick Actions
              </h2>
              <a href="/devices" className="view-all-link">
                View All →
              </a>
            </div>
                ...
                {recentDevices.length > 0 ? (
                  recentDevices.map((device) => (
                    <div key={device.id} className="activity-item">
                      ...
                      <span
                        className={`badge ${device.online ? "badge-success" : "badge-offline"}`}
                      >
